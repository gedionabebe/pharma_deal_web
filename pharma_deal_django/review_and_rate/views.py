    ...
    if request.session['status'] == 'logged_in':
        if request.POST['transaction_status'] == 'accepted':
            if request.POST['pharma_id'] == request.session['user_id']:
                ...
                return render(request, 'rate_and_review.html',{'pharma_id':pharma_id,'product_id':product_id,'status':status,'owner_name':owner_name,'product_name':product_name,'cost':cost})
